...
CSS = """
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  /* The phone crop is the hard one. The frame is 16:9 and the hero is nearly
     square there, so `cover` throws away most of the width and the copy ends up
     over whatever survives — on this photograph, a sunlit car body. The middle
     of the wash is heavier here than on the desktop version for exactly that
     reason, and the horizontal anchor is pulled right so it lands on the people
     rather than on a wing mirror. */
  .ph.bg::before{background-position:62% 42%}
  .ph.bg::after{content:"";position:absolute;inset:0;z-index:1;
      background:linear-gradient(180deg,rgba(6,14,32,.86) 0%,rgba(6,14,32,.60) 26%,
                 rgba(6,14,32,.70) 60%,rgba(6,14,32,.92) 100%)}
  ...
  .mixin{position:relative;overflow:hidden;border-radius:28px;isolation:isolate;
      min-height:min(106vw,620px);display:flex;flex-direction:column;
  ...
  @media(min-width:760px){ .mixin{padding:36px 42px 64px;min-height:min(78vh,780px);border-radius:34px} }
  ...
  /* No wash over the picture. There used to be a dark gradient across the whole
     frame to guarantee contrast; the photograph is the point, so the type earns
     its own contrast with a stacked shadow instead of dimming the image. */
  ...
  /* The shadow is stacked rather than single: a tight dark halo that hugs the
     letterform, then a wide soft one that lifts it off whatever is behind it.
     That is what keeps white type legible over the bright wall in the top half
     of the photograph now that nothing is dimming the photograph. */
  .mixsay h2{color:#fff;font-size:clamp(34px,6.2vw,72px);line-height:1.0;font-weight:800;
      letter-spacing:-.03em;text-transform:uppercase;
      text-shadow:0 0 10px rgba(0,0,0,.78),0 2px 5px rgba(0,0,0,.8),
      0 6px 30px rgba(0,0,0,.62),0 14px 70px rgba(0,0,0,.45)}
  .mixsay p{margin-top:16px;font-size:clamp(15.5px,1.5vw,19px);line-height:1.6;font-weight:600;
      color:#fff;
      text-shadow:0 0 8px rgba(0,0,0,.8),0 2px 4px rgba(0,0,0,.85),
      0 5px 24px rgba(0,0,0,.7),0 12px 55px rgba(0,0,0,.5)}
